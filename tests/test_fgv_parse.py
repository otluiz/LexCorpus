...
<html>
  <body>
    <a href="/concursos" class="active is-active">Concursos</a>
    <a href="https://conhecimento.fgv.br/concursos/nosso-portfolio#tabs">
      Nosso portfólio
    </a>
    <a href="/concursos/dataprev26" hreflang="pt-br">
      Concurso Público para a DATAPREV
    </a>
    <a href="/concursos/tjpe26" hreflang="pt-br">
      Concurso   Público para o   TJPE
    </a>
    <a href="/concursos/dataprev26" hreflang="pt-br">
      Concurso Público para a DATAPREV
    </a>
    <nav class="pager">
      <a href="?page=1" title="Ir para a próxima página" rel="next">próxima</a>
    </nav>
  </body>
</html>
"""
...
<html>
  <body>
    <a href="/concursos/seducsp26tecnico" hreflang="pt-br">
      Processo Seletivo SEDUC SP
    </a>
  </body>
</html>
"""
...
def test_parse_listagem_extrai_slugs_e_pagina():
    spider = FgvSpider(descoberta="1")
    url = "https://conhecimento.fgv.br/concursos"

    saida_p1 = list(spider.parse_listagem(make_response(url, HTML_LISTAGEM_P1)))

    # nav (/concursos, nosso-portfolio) fica de fora; duplicado conta uma vez
    assert sorted(spider._descobertos) == ["dataprev26", "tjpe26"]
    d = spider._descobertos["dataprev26"]
    assert d["banca"] == "fgv"
    assert d["rotulo"] == "Concurso Público para a DATAPREV"
    assert d["url"] == "https://conhecimento.fgv.br/concursos/dataprev26"
    # whitespace interno do rótulo é normalizado
    assert spider._descobertos["tjpe26"]["rotulo"] == (
        "Concurso Público para o TJPE"
    )

    # página 1 rende um follow para a próxima página
    assert len(saida_p1) == 1
    assert saida_p1[0].url == url + "?page=1"

    # última página (sem rel=next) não gera follow
    saida_p2 = list(
        spider.parse_listagem(make_response(url + "?page=1", HTML_LISTAGEM_P2))
    )
    assert saida_p2 == []
    assert "seducsp26tecnico" in spider._descobertos
